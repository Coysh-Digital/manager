...
Everything Manager knows about a site, that site told it. The Manager Connector is the plugin that
does the telling.
...
|---|---|
| Install or run updates | Manager reports that an update exists. Applying it is your deployment, on your schedule. |
...
implement — including things Manager might ask for. If a future Manager sent a job type this version
does not know, the site declines and says so.
...
database. That means pointing a site at a different Manager takes a deployment — which is the right
amount of friction for a setting that decides who your site reports to.
...
Manager generates a single-use enrolment code, good for fifteen minutes.

```bash
php craft manager-connector/pair mgr_enrol_xxxxxxxxxxxx
```

Or from the Craft control panel, under **Utilities → Manager Connector**, if you would rather not use
the command line. Either way needs the `utility:manager-connector` permission.

What happens:

1. The site generates an Ed25519 keypair. The private half is encrypted with Craft's own security key
   and stored in the site's database. It never leaves the server.
2. It sends the public half, the enrolment code and its hostname to Manager, over HTTPS.
3. Manager checks the code, records the public key, and signs its reply.
4. The site verifies that signature against the key Manager just offered, and against the nonce it
   sent. Trust on first use, but verified — the reply has to be signed by the key it is presenting.

From then on, every request the site makes is signed with that key. Manager only ever holds the public
half, so a Manager database compromise does not let anybody impersonate your sites.

If the hostname does not match what was entered in Manager, pairing is **held for confirmation**
rather than accepted. That is not an error — it is the expected result when a site is behind a CDN
under a different name, and it means a human looks at it. Approve it in Manager and the site starts
reporting.
...
That is not a policy, it is a schema. Every report is checked against a strict allowlist before it is
sent, and again when Manager receives it. A field that is not on the list gets the report **rejected**
rather than stripped — if this plugin ever started collecting more than it should, through a bad change
or a bad actor, the report fails at the door instead of quietly succeeding.
...
interval however busy the site is, and all a request does is push a queue job — the visitor waits for
nothing. It costs one cache read per request.

That works on hosting with no cron, which is most shared hosting.

If you have cron, it is more predictable. Turn off `webTrigger` and add:
...
Schedule all six even for capabilities you have not granted. Each one checks before it sends anything,
so an ungranted task is a no-op — and when you do grant it later, it just starts working.
...
Before any of that, the site checks the keys Manager offered against your pinned list. Anything
unexpected and it refuses **before dumping anything**, so a refused backup never leaves a copy of your
database on disk.
...
| `manager-connector/pair <code>` | Pair with Manager |
...
| `manager-connector/jobs` | Ask Manager whether there is anything to do |
...
This deletes the keypair from the site's database. It does not tell Manager, because a site that has
been compromised should not be able to remove itself from your dashboard — you would want to see it
sitting there having gone quiet.

So do both: disconnect on the site, then revoke the site in Manager. Two steps with two different
purposes.

If a site is compromised, revoke it in Manager **first**. That stops it being trusted immediately,
whatever is happening on the server.

## Troubleshooting

**Nothing appears in Manager after pairing.** Check `manager-connector/status`. If it says
`pending_confirmation`, the hostname did not match and somebody needs to approve it in Manager.

**Paired, but silent.** Either the queue is not running (with `webTrigger` on) or cron is not (with it
off). `php craft queue/run` will tell you which.
...
accepted from Manager. They are in `bin/verify-invariants.php` and they are short enough to read.
