...
backups are going and whether the edition label matches the code actually running. Most problems show
up in that output before you have to go looking.
...
**Migrations fail on start-up.** Check Postgres is actually up and reachable. Manager needs Postgres
15+, not MySQL — the audit log depends on triggers and revoked table privileges that MySQL has no
equivalent for.
...
Replay protection fails **closed**: if Manager cannot check whether it has seen a nonce before, it
refuses the request rather than accepting it and hoping. That is the correct behaviour and it is still
an outage.
...
at. `/up` is liveness only — it deliberately stays green during a brief Redis blip, because restarting
the container would turn a blip into a real outage.
...
designed behaviour, not a bug — approve it on the site's page in Manager, or correct the expected
domain and re-pair. [Pairing a site](/pairing) covers it.

**"could not verify the platform's response"** — something between the site and Manager is altering
response bodies. Usually a proxy. The site refuses to pair with something it cannot verify.
...
`MANAGER_TRUSTED_PROXIES` is not set, so Manager is refusing to believe the forwarded headers — which
is right, because trusting them from an untrusted source lets anybody spoof their address.

Set it to your proxy's address. Never to `*`. [Behind a reverse proxy](/reverse-proxy) has the detail.

## Backups

**"the platform has no artifact encryption key configured"** — the organisation has no active recovery
key, or the connector is too old to use them. Add one: [Recovery keys](/recovery-keys).
...
is why the message names the fingerprint. Check it against your own `.pub` file, not against Manager's
screen.
...
**"This platform cannot decrypt that artifact"** — working as intended. Backups are encrypted to your
recovery keys. Download it and use `manager-restore decrypt`. See [Restoring a
backup](/restoring).
...
- the hour or day does not match its schedule (times are in the **organisation's** time zone, not the
  server's);
...
Manager opens a TLS connection to the site's expected domain. If that fails it records the fact rather
than guessing at an expiry.

Common causes: the domain does not resolve from the Manager server; a firewall blocks outbound 443;
the expected domain is an internal name; or the domain resolves to a private address, which is refused
on purpose — a domain pointing at `169.254.169.254` would otherwise turn a monitoring check into a
request for cloud instance credentials.
...
Every Manager response carries a `Manager-Correlation-Id` header, and rejected connector requests log
their reason against that id. If a site reports a failure with a correlation id, that is the thing to
grep for.
